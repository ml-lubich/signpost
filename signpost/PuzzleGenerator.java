...
class PuzzleGenerator implements PuzzleSource {
    ...
    static Sq findUniqueSuccessor(Model model, Sq start) {
        ...
        int specialCaseDesperation = 0;
        int uniqueConnectables = 0;
        PlaceList currSetPls = model.allSuccessors(x, y, start.direction());
        if (currSetPls == null) {
            return null;
        }
        for (Place plInSet : currSetPls) {
            Sq sqrInSet = model.get(plInSet.x, plInSet.y);
            boolean tmp = sqrInSet.sequenceNum() == start.sequenceNum() + 1;
            boolean b = start.connectable(sqrInSet);
            if (start.group() != 0 && start.connectable(sqrInSet)) {
                outputCoords[0] = sqrInSet.pl.x;
                outputCoords[1] = sqrInSet.pl.y;
                uniqueConnectables += 1;
            } else if (start.group() == 0 && b && tmp) {
                outputCoords[0] = sqrInSet.pl.x;
                outputCoords[1] = sqrInSet.pl.y;
                return sqrInSet;
            }
        ...
        if (uniqueConnectables == 0) {
            for (Place plInSet : currSetPls) {
                Sq sqrInSet = model.get(plInSet.x, plInSet.y);
                boolean b = start.connectable(sqrInSet);
                if (start.group() == 0 && sqrInSet.group() != 0 && b) {
                    outputCoords[0] = sqrInSet.pl.x;
                    outputCoords[1] = sqrInSet.pl.y;
                    specialCaseDesperation += 1;
                }
            }
            Sq output = model.get(outputCoords[0], outputCoords[1]);
            if (specialCaseDesperation == 1) {
                return output;
            } else {
                return null;
            }
    ...
    static Sq findUniquePredecessor(Model model, Sq end) {
        ...
        int[] outputCoords = new int[2];
        int uniqueConnectables = 0;
        PlaceList predsEnd = end.predecessors();
        if (predsEnd == null) {
            return null;
        }
        for (Place plInSet : predsEnd) {
            Sq sqrInSet = model.get(plInSet.x, plInSet.y);
            if (sqrInSet.connectable(end)) {
                outputCoords[0] = sqrInSet.pl.x;
                outputCoords[1] = sqrInSet.pl.y;
                uniqueConnectables += 1;
            }
