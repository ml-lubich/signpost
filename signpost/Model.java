 ...
 *
 * -1 => unlinked and unnumbered sq's (grey in signpost)
 * 0 => sequence group
 * 1 or greater => player created groups (a, a+1) (b, b+1, b+2)
 *
...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        int[][] soln = solution();
        _solnNumToPlace = new Place[area + 1];
        _board = new Sq[_width][_height];
        for (int i = 0; i < width(); i++) {
            for (int j = 0; j < height(); j++) {
                int ad = arrowDirection(i, j);
                if (soln[i][j] != 1 && soln[i][j] != area) {
                    _board[i][j] = new Sq(i, j, 0, false, ad, -1);
                    addToBoard(i, j);
                }
                if (soln[i][j] == 1) {
                    _board[i][j] = new Sq(i, j, 1, true, ad, 0);
                    addToBoard(i, j);
                }
                if (soln[i][j] == area) {
                    _board[i][j] = new Sq(i, j, area, true, ad, 0);
                    addToBoard(i, j);
                }
            }
        }
        helper(solution);
        int a = 1;
        while (a < _solnNumToPlace.length) {
            if (_solnNumToPlace[a] == null) {
                throw new IllegalArgumentException("1-last do not all appear");
            }
            a += 1;
        }
        finishModel(area);
    ...
    /**
     * Helper function.
     * @param solution Double array.
     */
    public void helper(int[][] solution) {
        int k = 1; int a = 0; int b = 0;
        boolean loopBreaker = false;
        while (k < _solnNumToPlace.length) {
            a = 0;
            loopBreaker = false;
            while (a < _width && !loopBreaker) {
                b = 0;
                while (b < _height && !loopBreaker) {
                    int solnNum = solution[a][b];
                    if (solnNum == k) {
     ...
     * Finishes the model.
     * @param area */
    public void finishModel(int area) {
        PlaceList tmp;

        for (int i = 0; i < width(); i++) {
            for (int j = 0; j < height(); j++) {
                Sq currSq = _board[i][j];
                if (area != currSq._sequenceNum) {
                    tmp = _allSuccessors[i][j][arrowDirection(i, j)];
                    currSq._successors = tmp;

                    for (Place successorSq : currSq._successors) {
                        Sq succSq = this.get(successorSq);
                        if (succSq._predecessors == null) {
                            succSq._predecessors = new PlaceList();
                        }
                        succSq._predecessors.add(currSq.pl);
                    }
                }
            }
        }
        for (int i = 0; i < width(); i++) {
            for (int j = 0; j < height(); j++) {
                if (_board[i][j]._predecessor != null) {
                    tmp = _board[i][j]._predecessor._successors;
                    _board[i][j]._predecessors = tmp;
                }
            }
        }
    }
     ...
     * Makes copy of model.
     * @param model */
    public void makecopy(Model model) {
        ...
        for (int i = 0; i < width(); i++) {
            for (int j = 0; j < height(); j++) {
                int x = model.get(i, j).x;
                int y = model.get(i, j).y;
                int sm = model.get(i, j)._sequenceNum;
                int direction = model.get(i, j).direction();
                int groupNum = model.get(i, j)._group;
                boolean isFixedNum = model.get(i, j)._hasFixedNum;
                Sq tmp = new Sq(x, y, sm, isFixedNum, direction, groupNum);
                _board[i][j] = tmp;
        ...
        for (int i = 0; i < width(); i++) {
            for (int j = 0; j < height(); j++) {
                ...
                if (model.get(i, j)._successor != null) {
                    y = model.get(i, j)._successor.y;
                    x = model.get(i, j)._successor.x;
        ...
        for (int i = 0; i < width(); i++) {
            for (int j = 0; j < height(); j++) {
                int headX = model.get(i, j)._head.x;
                int headY = model.get(i, j)._head.y;
                _board[i][j]._head = _board[headX][headY];
            }
        }
    }
    ...
    boolean autoconnect() {
        ...
        for (int i = 0; i < _allSquares.size(); i++) {
            for (int j = 0; j < _allSquares.size(); j++) {
                Sq curr = _allSquares.get(i);
                Sq next = _allSquares.get(j);
                boolean tmp = next.sequenceNum() == 1 + curr.sequenceNum();
                if (curr.connectable(next) && tmp) {
                    curr.connect(next);
                    changesMade = true;
                }
            }
    ...
    void solve() {
        for (Sq currSq : _allSquares) {
            int x = currSq.pl.x; int y = currSq.pl.y;
            int seqNum = _solution[x][y];
            currSq._sequenceNum = seqNum;
            currSq._head._group = 0;
        }
        for (Sq currSq : _allSquares) {
            for (Sq nextSq : _allSquares) {
                if (nextSq._sequenceNum == currSq._sequenceNum + 1) {
                    nextSq._predecessor = currSq;
                    currSq._successor = nextSq;
                }
                if (currSq._sequenceNum == nextSq._sequenceNum - 1) {
                    nextSq._predecessor = currSq;
                    currSq._successor = nextSq;
                }
            }
        }
        int[] coordsFirstSq = new int[2];
        for (Sq currSq : _allSquares) {
            if (currSq._sequenceNum == 0) {
                coordsFirstSq[0] = currSq.x;
                coordsFirstSq[1] = currSq.y;
                break;
            }
        }
        Sq firstSq = this.get(coordsFirstSq[0], coordsFirstSq[1]);

        for (Sq currSq : _allSquares) {
            if (currSq._head != firstSq) {
                currSq._head = firstSq;
            }
        }

        _usedGroups = new HashSet<>();
        _unconnected = 0;
    }
    ...
    final class Sq {
        ...
        /**
         * Checks if 2 squares are connectable.
         * @param s1 square
         * @return */
        boolean connectable(Sq s1) {
            /** currSquare (this) to S1 */
            int lastSeqNum = width() * height();
            if (this.direction() == dirOf(this.x, this.y, s1.x, s1.y)) {
                int s = this.sequenceNum();
                boolean b = this.successor() == null;
                boolean t = (s1.sequenceNum() != 1 && s != lastSeqNum);
                if ((s1.predecessor() == null && b) && t) {
                    if (s1.sequenceNum() != 0 && this.sequenceNum() != 0) {
                        return (this.sequenceNum() == s1.sequenceNum() - 1);
                    } else if (s1.sequenceNum() == 0 && s == 0) {
        ...
        boolean connect(Sq s1) {
            ...
            Sq tmp = this;
            boolean thisWasUnumbered = false;
            boolean s1WasUnumbered = false;
            if (this.group() != 0) {
                thisWasUnumbered = true;
            }
            if (s1.group() != 0) {
                s1WasUnumbered = true;
            }
            if (tmp._sequenceNum != 0 && s1WasUnumbered) {
                while (tmp._successor != null) {
                    tmp._successor._sequenceNum = tmp._sequenceNum + 1;
                    tmp = tmp._successor;
                }
                releaseGroup(sgroup);
            }
            Sq tmpS1 = s1;
            if (s1._sequenceNum != 0 && thisWasUnumbered) {
                while (tmpS1._predecessor != null) {
                    tmpS1._predecessor._sequenceNum = tmpS1._sequenceNum - 1;
                    tmpS1 = tmpS1._predecessor;
                }
            ...
            tmp = this;
            while (tmp._successor != null) {
                tmp._successor._head = this._head;
                tmp = tmp._successor;
            }
            boolean b = s1._sequenceNum == 0;
            if (this._sequenceNum == 0 && b) {
                int t = thisgroup;
                this._head._group = joinGroups(t, sgroup);
            }
            return true;
        }
        ...
        void disconnect() {
            ...
            if (this._sequenceNum == 0) {
                ...
                flagFixed = false;
                while (tmpNext != null) {
                    if (tmpNext._hasFixedNum) {
                        flagFixed = true; break;
                    }
                    tmpNext = tmpNext._successor;
                }
                next._head = next;
                if (!flagFixed) {
                    tmp = next;
        ...
        /**
         * Does disconnect on one group.
         * @param next */
        public void oneGroup(Sq next) {
            boolean b = next._successor == null;
            if (this._predecessor == null && next._successor == null) {
                releaseGroup(this.group());
                next._head = next;
                this._head._group = -1;
                next._head._group = -1;
            } else if (this._predecessor == null ^ b) {
                if (next._successor == null) {
                    next._head = next;
                    next._head._group = -1;
                }
                ...
                next._head = next;
                Sq tmp = next._successor;
                while (tmp != null) {
                    tmp._head = next; tmp = tmp._successor;
                }
                next._head._group = newGroup();
            }
        }
        ...
        public boolean equals(Object obj) {
            Sq sq = (Sq) obj;
            boolean b = sq._predecessor == null;
            return sq != null
                    && pl == sq.pl
